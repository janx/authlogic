      ...
      module Methods
        def self.included(klass)
          ...
          klass.class_eval do
            ...
            scope :logged_in, lambda{ gt(last_request_at: logged_in_timeout.seconds.ago).where(:current_login_at.ne => nil) }
            scope :logged_out, lambda{ where(:$or => [{:last_request_at => nil}, {:$lte => {:last_request_at => logged_in_timeout.seconds.ago}}]) }
          end
        end
